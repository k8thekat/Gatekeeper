...
class Whitelist_module(commands.Cog):
    def __init__(self,AMP,AMPservers,DB,DBConfig):
        self.AMP = AMP
...
#Used to add users to the DB when they request whitelist in the WL channel if Autowhitelist is False.
    async def whitelistMSGHandler(self,message,async_loop):
        ...
        status = asyncio.run_coroutine_threadsafe(self.whitelistUserCheck(curserver,user.IngameName),async_loop)
        if status == False:
    ...
    async def whitelistListCheck(self,client):
        logging.info('Whitelist Wait List Check...')

        wl_channel = self.DB.Whitelistchannel 
        if wl_channel == None:
            return False
        ...
        for index in range(0,len(self.WhitelistWaitList)):
            ...
            if not config.Randombotreplies:
                try:
                    await user['Context'].channel.send(config.Botwhitelistreplies[self.AMPservers[user['server'].InstanceID].Index])
                    await discord_user.add_roles(int(user['server'].DiscordRole),reason='Auto Whitelist')
                    user['server'].AddUser(user['User'])
                    return
                except Exception as e:
                    print(e)
                    await user['Context'].channel.send(f'{user["User"].name} you have been whitelisted on {user["server"].FriendlyName}.')
                    return
